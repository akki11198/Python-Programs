...
visualise = True
iou_path = open('/home/user/Desktop/Object_Detection/Code/frcnn-from-scratch-with-keras-master/mycords.txt', 'w+')
visualise = True
#path_Rect_txt = ''
...
for line in fo:
	...
	img_name = (filename.strip().split('/'))[7]# to know the name of file at index number
	st = time.time()
	filepath = filename
...
for key in bboxes:
        ...
        #print(IOU)
		cv2.imwrite('/home/user/Desktop/Object_Detection/Code/frcnn-from-scratch-with-keras-master/test/' + img_name ,img)
	# Find Intersection over Union
		IOU = abs(iou.bb_intersection_over_union(ground_truth_box, predict_box))
cv2.putText(img, "IoU: {:.4f}".format(IOU), (50, 50), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (0, 255, 0), 1)
